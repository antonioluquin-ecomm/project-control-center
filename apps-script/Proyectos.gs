...
function createProyecto_(params, user) {
  ...
  const estado      = optionalEnum_(params.estado, 'estado', getCatCached_(CATALOGOS.CAT_ESTADOS_PROYECTO, ESTADOS_PROYECTO), 'Por Hacer');
  const prioridad   = optionalEnum_(params.prioridad, 'prioridad', getCatCached_(CATALOGOS.CAT_PRIORIDADES, PRIORIDADES), 'Medium');
  const sitio       = params.sitio ? validateEnum_(params.sitio, 'sitio', getCatCached_(CATALOGOS.CAT_SITIOS, SITIOS)) : '';
...
function updateProyecto_(params, user) {
  ...
  if (params.estado !== undefined)        updates.estado = validateEnum_(params.estado, 'estado', getCatCached_(CATALOGOS.CAT_ESTADOS_PROYECTO, ESTADOS_PROYECTO));
  ...
  if (params.sitio !== undefined)         updates.sitio = params.sitio ? validateEnum_(params.sitio, 'sitio', getCatCached_(CATALOGOS.CAT_SITIOS, SITIOS)) : '';
