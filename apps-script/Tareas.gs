...
function createTarea_(params, user) {
  ...
  const tipo      = optionalEnum_(params.tipo, 'tipo', getCatCached_(CATALOGOS.CAT_TIPOS_TAREA, TIPOS_TAREA), 'Tarea');
  const estado    = optionalEnum_(params.estado, 'estado', getCatCached_(CATALOGOS.CAT_ESTADOS_TAREA, ESTADOS_TAREA), 'Por Hacer');
  const prioridad = optionalEnum_(params.prioridad, 'prioridad', getCatCached_(CATALOGOS.CAT_PRIORIDADES, PRIORIDADES), 'Medium');
  ...
  const area    = optionalEnum_(params.area, 'area', getCatCached_(CATALOGOS.CAT_AREAS, AREAS), '');
  ...
  const seccion = optionalEnumList_(params.seccion, 'seccion', getCatCached_(CATALOGOS.CAT_SECCIONES, SECCIONES));
...
function updateTarea_(params, user) {
  ...
  if (params.tipo !== undefined)        updates.tipo = validateEnum_(params.tipo, 'tipo', getCatCached_(CATALOGOS.CAT_TIPOS_TAREA, TIPOS_TAREA));
  if (params.estado !== undefined)      updates.estado = validateEnum_(params.estado, 'estado', getCatCached_(CATALOGOS.CAT_ESTADOS_TAREA, ESTADOS_TAREA));
  if (params.prioridad !== undefined)   updates.prioridad = validateEnum_(params.prioridad, 'prioridad', getCatCached_(CATALOGOS.CAT_PRIORIDADES, PRIORIDADES));
  ...
  // S6: dimensiones y enlaces externos.
  if (params.area !== undefined)        updates.area = params.area ? validateEnum_(params.area, 'area', getCatCached_(CATALOGOS.CAT_AREAS, AREAS)) : '';
  ...
  // Dimensiones reutilizables (Informe de Gestion / Requerimiento).
  if (params.seccion !== undefined) updates.seccion = optionalEnumList_(params.seccion, 'seccion', getCatCached_(CATALOGOS.CAT_SECCIONES, SECCIONES));
