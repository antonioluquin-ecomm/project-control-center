...
function createTarea_(params, user) {
  ...
  const tipo      = optionalEnum_(params.tipo, 'tipo', TIPOS_TAREA, 'Tarea');
  const estado    = optionalEnum_(params.estado, 'estado', ESTADOS_TAREA, 'Por Hacer');
...
function updateTarea_(params, user) {
  ...
  if (params.tipo !== undefined)        updates.tipo = validateEnum_(params.tipo, 'tipo', TIPOS_TAREA);
  if (params.estado !== undefined)      updates.estado = validateEnum_(params.estado, 'estado', ESTADOS_TAREA);
